...
class RM
{
	...
	public function getByNum($no_RM)
	{
		$stmt = $this->con->prepare("SELECT * FROM rm WHERE no_RM=:no_RM");
	...
	public function insertRM($id_pendaftaran,$ruang,$mrs,$jam,$anamnesa,$riwayat_penyakit,$riwayat_pekerjaan,$riwayat_alergi,$keadaan_umum,$kesadaran,$E,$V,$M, $suhu, $nadi, $respirasi, $TD, $pemeriksaan, $penunjang, $diagnosa_kerja, $diagnosa_banding, $pelayanan, $nama_dr){
	...
	public function updateRM($id,$nama,$jen_kelamin,$alamat,$ruang,$nama_rs,$mrs,$jam,$anamnesa,$riwayat_penyakit,$riwayat_pekerjaan,$riwayat_alergi,$keadaan_umum,$kesadaran,$E,$V,$M, $suhu,$nadi, $respirasi, $TD, $pemeriksaan, $penunjang, $diagnosa_kerja, $diagnosa_banding, $pelayanan, $nama_dr)
	{		
		...
		// $no_RM = $NIK;
		$stmt = $this->con->prepare("UPDATE rm SET nama=:nama,jen_kelamin=:jen_kelamin,umur=:umur,alamat=:alamat,ruang=:ruang,nama_rs=:nama_rs,mrs=:mrs,jam=:jam,anamnesa=:anamnesa,riwayat_penyakit=:riwayat_penyakit,riwayat_pekerjaan=:riwayat_pekerjaan,riwayat_alergi=:riwayat_alergi,keadaan_umum=:keadaan_umum,kesadaran=:kesadaran,E=:E,V=:V,M=:M,suhu=:suhu, nadi=:nadi, respirasi=:respirasi,TD=:TD,pemeriksaan=:pemeriksaan,penunjang=:penunjang,diagnosa_kerja=:diagnosa_kerja,diagnosa_banding=:diagnosa_banding,pelayanan=:pelayanan,nama_dr=:nama_dr,update_at=NOW() WHERE id_pendaftaran=:id");
	...
	public function deleteRM($no_RM)
	{
		$stmt = $this->con->prepare("DELETE FROM rm WHERE no_RM=:no_RM");
		$stmt->execute(array(":no_RM"=>$no_RM));
		return $stmt;
	}
